...
# AttentionPool2d (OpenAI CLIP) is only needed for the optional CLIP
# knowledge-distillation path (clip_kd). Import it lazily so this module — and the
# bcosification/densenet experiment models that import it — load without the CLIP repo.
try:
    from CLIP.clip.model import AttentionPool2d
except ImportError:
    AttentionPool2d = None
    ...
    def bcosify(cls, model, model_config):
        ...
        for n, module in model.named_children():
            if len(list(module.children())) > 0:
                if clip_kd and n == 'attnpool' and AttentionPool2d is not None and isinstance(module, AttentionPool2d):
