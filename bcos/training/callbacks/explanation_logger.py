...
class ExplanationsLogger(pl_callbacks.Callback):
    ...
    def setup(
        ...
        # Labels can be:
        #  - scalar class index (binary/multiclass)
        #  - multi-hot vector [C] (multilabel, e.g. VinBig)
        if len(lbls) > 0 and isinstance(lbls[0], torch.Tensor):
            self.lbls = torch.stack([l.detach().cpu() for l in lbls])
        else:
    ...
    def get_results(self):
        ...
        with torch.enable_grad(), self.model.explanation_mode():
            ...
            for img, lbl in zip(imgs, lbls):
                ...
                # lbl can be scalar (binary/multiclass) or a vector (multilabel)
                ...
                # For multilabel: choose the highest-probability label as the representative "pred"
                # and backprop that logit for visualization.
                if lbl_t.ndim > 0:
                    probs = torch.sigmoid(out)[0]
                    pred_idx = int(torch.argmax(probs).item())
                    score = out[0, pred_idx]
                    score.backward(inputs=[img])
                    pred = pred_idx
                    lbl_out = lbl_t.detach().cpu()
                else:
                    pred_pair = out.max(1)
                    pred_pair.values.backward(inputs=[img])
                    pred = int(pred_pair.indices.item())
                    lbl_out = int(lbl_t.item())
        ...
        for i, (img, lbl, pred, weight) in enumerate(results):
            ...
            for ax in (expl_ax, contrib_ax):
                for spine in ax.spines.values():
                    # Binary/multiclass: lbl is int. Multilabel: lbl is a vector.
                    if isinstance(lbl, torch.Tensor) or isinstance(lbl, np.ndarray):
                        # correct if the predicted label is present in the multi-hot target
                        try:
                            is_correct = float(lbl[pred]) > 0.5
                        except Exception:
                            is_correct = False
                    else:
                        is_correct = (lbl == pred)

                    spine.set_edgecolor("lime" if is_correct else "orange")
                    # spine.set_linewidth(3)  # Set the width of the spine
                    spine.set_linestyle((0, (5, 3)) if is_correct else (0, (2, 3)))
