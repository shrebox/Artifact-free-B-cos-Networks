    ...
    if pooling_type is not None:
        ...
        if pooling_type == "BlurPool":
            ...
            model.classifier.linear = NormedConv2d(
                1024, 14, kernel_size=(1, 1), stride=(1, 1), bias=False
            ...
            if pretrained:
                print("Loading pretrained B-cos densenet121...")
            else:
                print("Loading random init B-cos densenet121...")
            model = torch.hub.load(
                "B-cos/B-cos-v2", "densenet121", pretrained=pretrained
            )
            model.classifier.linear = NormedConv2d(
                1024, 14, kernel_size=(1, 1), stride=(1, 1), bias=False
            )
