...
class LRSchedulerFactory:
    def __init__(
        ...
        assert name.lower() != "reduceonplateau" or interval == "epoch", (
            "ReduceLROnPlateau is only supported with interval='epoch' (it expects a monitored metric)."
        )
        assert (
        ...
        assert name.lower() != "reduceonplateau" or (
            self.warmup_epochs is None and self.warmup_steps is None
        ), "Warmup is not supported for ReduceLROnPlateau in this factory."
        assert (
    ...
    def create(
        ...
        if name == "steplr":
            ...
            raise RuntimeError(
                f"Invalid lr scheduler '{name}'. Only StepLR, CosineAnnealingLR, ExponentialLR and ReduceLROnPlateau "
                "are supported."
            )
        ...
        if name == "reduceonplateau":
            # Lightning expects monitor to be provided for ReduceLROnPlateau
            return dict(
